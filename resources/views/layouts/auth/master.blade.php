...
<body class="hold-transition sidebar-mini">
    <div class="wrapper">
        <nav class="main-header navbar navbar-expand navbar-white navbar-light">
            ...
            <aside class="main-sidebar sidebar-dark-primary elevation-4 side-gradient">
                <a href="{{ route('home') }}" class="brand-link d-flex flex-column align-items-center" style="padding: 6px;background-image: url(https://hundee.torzo.in/admin/dist/img/stamp.jpg);background-size: 126px;">
                    ...
                    <img src="{{ asset('admin/dist/img/logo.png') }}" alt="AdminLTE Logo" style="height: 44px">
                </a>

                <div class="sidebar">
                    <div class="user-panel mt-3 pb-1 mb-1">
                        <div class="d-flex">
                            <div class="image">
                        ...
                        <h6 class="mx-3 mt-2 d-block badge badge-sm bg-{{App\Models\UserType::find(Auth::user()->user_type)->color}}">{{ ucwords(implode(' ',explode('_',App\Models\UserType::find(Auth::user()->user_type)->name))) }}</h6>
                    </div>
                    ...
                    <nav class="mt-2">
                        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu"
                            ...
                            <li class="nav-item">
                                <a href="{{ route('user.borrower.list',['show'=>'borrowers']) }}"
                                    class="nav-link {{ request()->is('user/borrower*') && request()->query('show') == 'borrowers' ? 'active' : '' }}">
                            ...
                            @if (auth()->user()->user_type == 1)
                                <li class="nav-item link-dropdown">
                                    <a href="javascript:void(0)"
                                        class="nav-link {{ request()->is('user/docu-checker*') ? 'active' : '' }}">
                                        <i class="nav-icon fas fa-users-cog"></i>
                                        <p>Docu Checker</p>
                                    </a>

                                    <ul class="list-unstyled p-0 m-0 sub-menu">
                                        <li><a href="{{ route('user.docu-checker.customer') }}">All Customers</a></li>
                                        <li><a href="{{ route('user.customer.enquiry-list') }}">All Enquiries</a></li>
                                    </ul>
                                </li>
                            @endif
                            <li class="nav-item">
                                <a href="{{ route('user.borrower.list',['show'=>'borrower & agreement']) }}"
                                    class="nav-link {{ request()->is('user/borrower*') && request()->query('show') == 'borrower & agreement' ? 'active' : '' }}">
                                    <i class="nav-icon fas fa-users-cog"></i>
                                    <p>Borrowers</p>
                                </a>
                            </li>
                            ...
                            @if (auth()->user()->user_type == 1)
                                ...
                                <li class="nav-item">
                                    <a href="{{ route('esigning-list') }}"
                                        class="nav-link {{ request()->is('esigning-list*') ? 'active' : '' }}">
                                        <i class="nav-icon fas fa-edit"></i>
                                        <p>Esigning Management</p>
                                    </a>
                                </li>
                            @endif
                            @if (auth()->user()->user_type == 1)
                            ...
                            {{-- @if (auth()->user()->user_type == 1)
                            <li class="nav-item">
                                <a href="{{ route('user.fsd.list') }}"
                                    class="nav-link {{ request()->is('user/fsd*') ? 'active' : '' }}">
                            ...
                            @if (auth()->user()->user_type == 1 || auth()->user()->user_type == 2)
                                ...
                                {{-- <li class="nav-item">
                                    <a href="{{ route('user.emp.list',['term'=>"Relationship Manager"]) }}"
                                        class="nav-link {{ request()->is('user/employee*') && request()->query('term') == 'Relationship Manager'  ? 'active' : '' }}">
                                        <i class="nav-icon fas fa-users"></i>
                                        <p>RM Master</p>
                                    </a>
                                </li> --}}
                            ...
                            @if (auth()->user()->user_type == 1 || auth()->user()->user_type == 2)
                                <li class="nav-item">
                                    <a href="{{ route('user.qualification.list') }}"
                                        class="nav-link {{ request()->is('user/qualification*') ? 'active' : '' }}">
                                        <i class="nav-icon fas fa-university"></i>
                                        <p>Highest qualification management</p>
                                    </a>
                                </li>
                            @endif  
                            <li class="nav-item">
    ...
    <script>
        ...
        @elseif (Session::has('error'))
            toastFire('danger', '{{ Session::get('message') }}');
        @elseif (Session::has('Warning'))
            toastFire('danger', '{{ Session::get('Warning') }}');
        @endif
        ...
        var onesDigit = ['', 'One ', 'Two ', 'Three ', 'Four ', 'Five ', 'Six ', 'Seven ', 'Eight ', 'Nine ', 'Ten ', 'Eleven ', 'Twelve ', 'Thirteen ', 
        ...
        const navDropdown = document.querySelector('.link-dropdown')
