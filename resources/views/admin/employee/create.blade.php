                ...
                <div class="card">
                    <div class="card-header">
                        <div class="card-tools">
                            ...
                            <a href="{{route('user.emp.list')}}" class="btn btn-sm btn-primary"> <i class="fas fa-chevron-left"></i> Back</a>
                        </div>
                    </div>
                    <div class="card-body">
                        <form class="form-horizontal" method="POST" action="{{ route('user.emp.store') }}" id="profile-form">
                        @csrf
                            <div class="form-group row">
                                ...
                                <div class="col-sm-6">
                                    <label for="employee_id" class="col-form-label">Employee Code <span class="text-danger">*</span></label>
                            ...
                            <div class="form-group row">
                                <div class="col-sm-6">
                                    <label for="reporting_manager" class="col-form-label">Manager<span class="text-danger">*</span></label>
                                    <select name="reporting_manager" id="reporting_manager" class="form-control @error('reporting_manager') {{'is-invalid'}} @enderror">
                                        <option value="" selected disabled>Select Reporting Manager</option>
                                        <option value="">No Reporting Manager</option>
                                        @foreach ($data->users as $item)
                                            <option value="{{$item->id}}" {{(old('reporting_manager') == $item->id) ? 'selected' : '' }}>{{$item->name}}</option>
                                        @endforeach
                                    </select>
                                    @error('reporting_manager') <p class="small mb-0 text-danger">{{$message}}</p> @enderror
                                </div>
                                <div class="col-sm-6">
                            ...
                            <div class="form-group row">
                                <div class="col-sm-12">
                                    ...
                                    <textarea name="street_address" id="street_address" cols="30" rows="2" class="form-control"></textarea>
                                </div>
                            </div>
                            <div class="form-group row">
                                <div class="col-sm-12">
