                ...
                <div class="card">
                    <div class="card-header">
                        <div class="card-tools">
                            ...
                            <a href="#CreateModal" class="btn btn-sm btn-primary" data-toggle="modal"><i class="fas fa-plus"></i> Create Ducument Type</a>
                            <a href="{{route('user.document.rejectionreason-list')}}" class="btn btn-sm btn-primary"><i class="fas fa-plus"></i> Rejection Reasons List</a>
                            <a href="{{route('user.customer.additional-list')}}" class="btn btn-sm btn-primary"><i class="fas fa-plus"></i> Additional Documents List</a>
                        </div>
                    </div>
                    <div class="card-body">
                        ...
                        <table class="table table-sm table-bordered table-hover" id="showOfficeTable">
                            ...
                            <tbody>
                                ...
                                <tr id="tr_{{ $value->id }}">
                                    <td>{{ $key+1 }}</td>
                                    <td width="80%">{{ $value->document_name }}</td>
                                    {{-- <td>
                                        <ul>
                                            @foreach($getDocument as $document)
                                            <li>{{ $document->document_name}} <span><a href="#EditDocNameModal{{ $document->id }}" class="badge badge-dark action-button" data-toggle="modal" title="Edit">Edit</a></span></li>
                                            {{-- Modal --}}
                                            
                                            {{-- <div class="modal fade pr-0" id="EditDocNameModal{{ $document->id }}" tabindex="-1" role="dialog" aria-hidden="true">
                                                        ...
                                                        <form method="post">
                                                            ...
                                                            <div class="modal-body">
                                                                ...
                                                                <div class="mb-3">
                                                                    ...
                                                                    <textarea name="document_name" id="document_name{{ $document->id }}" class="form-control">{{ $document->document_name }}</textarea>
                                                                </div>
                                                            </div>
                                                            <div class="modal-footer text-right">
                                                                <button type="submit" class="btn btn-sm btn-success" onclick="EditDeta1ls(event,{{ $document->id }})">Save changes <i class="fas fa-upload"></i> </button>
                                                            </div>
                                                        </form>
                                                    </div>
                                                </div>
...
<script>
    ...
    var document_name = $('#document_name').val();
      $.ajax({type: "POST",
            ...
            success:function(result){
                if(result.status ==200){
                    $('#alertSms').addClass('badge badge-success').html("Added New Document Type");
                    setTimeout(function () {
