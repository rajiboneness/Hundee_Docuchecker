                    ...
                    <div class="card-body">
                        <table class="table table-sm table-bordered table-hover" id="borrowers-table">
                            ...
                            <tbody>
                                ...
                                $DocumentName = array();
                                @endphp
                                @foreach($Finalparent as $Pkey => $ParentVal)
                                    @php
                                    $Parent = App\Models\Document::where('id', $ParentVal)-> first();
                                   @endphp
                                    <td colspan="4" class="parentName text-light px-4">{{$Parent->document_name}}</td>
                                    @foreach($TotalDocument_id as $Dkey => $DocuId)
                                        ...
                                        @endphp
                                        @if(isset($Document))
                                            {{-- @foreach($Document as $Dkey => $DValue) --}}
                                            @php
                                ...
                                @if(count($AdditionalRejectDocument)>0)
                                    <td colspan="4" class="parentName text-light px-4">Additional Documents</td>
                                    ...
                                    @endphp
                                        <tr>
                                            ...
                                            <td>{{ date('d M Y', strtotime($Reject->created_at)) }}</td>
                                            <td>
                        ...
                        <!-- Modal -->
                            ...
                            @foreach($pendingData as $pendingKey => $pendingValue)
                                ...
                                if($pendingValue->status==4){
                                    $RejectDocument = App\Models\RejectDocument::select('document_reason')->latest('created_at')->where('customer_loan_documents_id', $pendingValue->id)->first();
                                    // foreach($RejectDocument as $RejectData){
                                        $dataArray[] = $RejectDocument->document_reason;
                                        $dataArray[] = 'Rejected';
                                    // }
                                }
