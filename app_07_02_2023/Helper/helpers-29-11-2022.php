    ...
    if(count($agreements) > 0){
        foreach ($agreements as $value) {
            if(BorrowerAgreement::find($value->id)->application_id != null)
                echo "<i class='text-sm text-bold'>".BorrowerAgreement::find($value->id)->application_id."</i><br>";
            else
            echo "<i class='text-sm text-muted'>No App.ID</i><br>";
        }
    }
    else{
        echo "<i class='text-sm text-muted'>No agreement</i>";
    }
}

function getEsignDetails($rfq_id)
{
    return ESignature::where('rfq_id',$rfq_id)->get();
}
