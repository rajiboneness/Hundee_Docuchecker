...
Route::get('/test', function() {
    $data = \DB::table('field_parent_relations')->get();

    foreach($data as $index => $item) {
        if ($item->id == 500) {
            break;
        } else {
            \DB::table('field_parent_relations')
            ->where('id', $item->id)
            ->update([
                'child_position' => $item->child_id    
            ]);
        }
    }
    // return $data;
...
Route::get('/zoop/borrower/{id}', [ZoopController::class, 'borrower'])->name('zoop.borrower');
