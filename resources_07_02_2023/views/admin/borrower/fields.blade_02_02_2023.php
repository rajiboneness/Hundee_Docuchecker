                                ...
                                <div class="col-md-6 text-sm-left">
                                    <div class="mb-2">
                                        ...
                                        if ($CustomerLoanDocument == 0) {
                                            ...
                                            $DocumentsPending = App\Models\CustomerLoanDocument::where('borrower_id', $data->borrower_id)->where('status', 1)->get();

                                            if (count($DocumentsPending) > 0) {
                                                $color = 'danger';
                                                $icon = '<i class="fas fa-info-circle mr-2"></i>';
                                            } else {
                                                $color = 'success';
                                        ...
                                        <a href="{{ $CustomerLoanDocument > 0 ? "#DocuChekerModal" : route('user.borrower.setAnnexure5doc', [$id]) }}"
                                            class="mb-1 btn {{auth()->user()->user_type == 1 || auth()->user()->user_type == 3 ? '' : 'disabled'}} btn-sm btn-{{ $data->annX5dataCount == 0 ? 'danger' : 'success' }}" data-toggle="{{ $CustomerLoanDocument > 0 ? "modal" : "" }}" rel="1" id="{{ route('user.borrower.setAnnexure5doc', [$id]) }}" onclick="GetDetails(this.id,this.rel)">
                                        ...
                                        <a href="javascript:void(0)" class="mb-1 btn {{auth()->user()->user_type == 1 || auth()->user()->user_type == 3 ? '' : 'disabled'}} btn-sm btn-{{CheckVernaculationDataSet($id) ? 'success' : 'danger'}}" data-toggle="modal"
                                            data-target="{{ $CustomerLoanDocument > 0 ? "#DocuChekerModal" : "#vernacularDeclarationModal" }}" id="#vernacularDeclarationModal" rel="2" onclick="GetDetails(this.id,this.rel)">
                                        ...
                                        <div class="modal fade" id="DocuChekerModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                            <div class="modal-dialog" role="document">
                                              <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <span><strong>{{ $CustomerLoanDocument }} Document{{ $CustomerLoanDocument > 1 ? "s are" : " is" }} not submitted yet</strong></span><br>
                                        ...
                                        @if ($data->stampPaper1 && $data->stampPaper2 && $data->stampPaper3 && $data->stampPaper4)
                                            ...
                                            <a href="{{ $CustomerLoanDocument > 0 ? "#DocuChekerModal" : "#viewPdfModal" }}" data-toggle="modal" class="btn btn-sm btn-success {{auth()->user()->user_type == 1 || auth()->user()->user_type == 3 ? '' : 'disabled'}}" rel="3" id="#viewPdfModal" onclick="GetDetails(this.id,this.rel)">
                                                <i class="fas fa-check-circle mr-2"></i> Stamp Paper
                                            </a>
                                        @else
                                            <a href="{{ $CustomerLoanDocument > 0 ? "#DocuChekerModal" : "#viewPdfModal" }}" data-toggle="modal" class="btn btn-sm btn-danger {{auth()->user()->user_type == 1 || auth()->user()->user_type == 3 ? '' : 'disabled'}}" rel="4" id="#viewPdfModal" onclick="GetDetails(this.id,this.rel)">
