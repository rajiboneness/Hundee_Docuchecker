...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach($agreementData as $dataValue){
            $value = BorrowerAgreement::where('borrower_id', $dataValue->id)->where('application_id', '!=', null)->first();
            if(!empty($value)){
        ...
        // Comptlete & Incomplete Agreement reports
        // $allresponse = DB::table('zoop_webhook_response')->select('esign_uniq_id')->groupBy('esign_uniq_id')->where('esign_uniq_id', '!=', null)->get();
        // dd($allresponse);
        // foreach($allresponse as $items){
        //     // dd()
        //         $gete_signatures = DB::table('e_signatures')->where('unique_id', $items->esign_uniq_id)->first();
        //         dd($gete_signatures);
        // }
