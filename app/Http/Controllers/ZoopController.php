...
class ZoopController extends Controller
{
    ...
    public function detail(Request $request, $borrowerId, $borrowerAgreementId, $rfqId) {
        ...
        $all_auth_sig = User::where('user_type', 1)->orWhere('user_type', 2)->where('id', '!=', 1)->get();
        ...
        $agreement_file = AgreementRfq::find($rfqId)->final_agreement;
        ...
        return view('admin.borrower.esign', compact('data', 'all_auth_sig', 'rfqId', 'e_signatures','b_checked','cb_checked','cb2_checked','as_checked','b_disabled','cb_disabled','cb2_disabled','as_disabled','submit_disabled','borrowerAgreementId','app_id','agreement_file'));
        return view('admin.borrower.esign', compact('data', 'all_auth_sig', 'rfqId', 'e_signatures','b_checked','cb_checked','cb2_checked','as_checked','b_disabled','cb_disabled','cb2_disabled','as_disabled','submit_disabled','borrowerAgreementId','app_id','agreement_file'));
    }
    ...
    {   
        ...
        if($request->executant){
            ...
            if (!empty($request->executant_name)) {
    ...
    public function MultipleEsign($id)
    {
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://live.zoop.one/contract/esign/v5/init',
    ...
    public function submitFinalAgreement(Request $request)
    {
        $agreement = fileUpload($request->final_agreement,'final-agreement');
        $save = AgreementRfq::where('id',$request->rfq_id)->update(['final_agreement'=>$agreement]);
        if($save){
            return redirect()->back()->with('success','Agreement uploaded successfully!');
        }else{
            return redirect()->back()->with('success','Some error occoured!');
        }
    }
}
