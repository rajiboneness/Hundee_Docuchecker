...
function form3lements($field_id, $name, $type, $value=null, $key_name, $required=null, $borrowerId=null, $form_type=null)
{
    ...
    $extraClass = 'text-uppercase'; // extra class name for filtering
    if (!empty($borrowerId)) {
            ...
            switch($key_name){
                ...
                case 'officiallyvaliddocumentsoftheborrower' :
                    ...
                    if ($form_type == 'show') {
                        ...
                        $rfq = AgreementRfq::select('id')->where('borrower_id', $borrowerId)->orderBy('id','DESC')->first();
                        if ($rfq) {
                ...
                case 'postdatecheque2' :
                    if ($form_type == 'show') {
                        ...
                        if ($rfq) {
                            ...
                            ($cheque2desc) ? $cheque2value1 = ($cheque2desc->field_value == null || '' ? 'PEERLESS FINANCIAL SERVICES LIMITED' : $cheque2desc->field_value) : $cheque2value1 = 'PEERLESS FINANCIAL SERVICES LIMITED';
                ...
                case 'postdatecheque3' :
                    if ($form_type == 'show') {
                        ...
                        if ($rfq) {
                            ...
                            ($cheque3desc) ? $cheque3value1 = ($cheque3desc->field_value == null || '' ? 'PEERLESS FINANCIAL SERVICES LIMITED' : $cheque3desc->field_value) : $cheque3value1 = 'PEERLESS FINANCIAL SERVICES LIMITED';
                ...
                case 'postdatecheque4' :
                    if ($form_type == 'show') {
                        ...
                        if ($rfq) {
                            ...
                            ($cheque4desc) ? $cheque4value1 = ($cheque4desc->field_value == null || '' ? 'PEERLESS FINANCIAL SERVICES LIMITED' : $cheque4desc->field_value) : $cheque4value1 = 'PEERLESS FINANCIAL SERVICES LIMITED';
                ...
                case 'postdatecheque5' :
                    if ($form_type == 'show') {
                        ...
                        if ($rfq) {
                            ...
                            ($cheque5desc) ? $cheque5value1 = ($cheque5desc->field_value == null || '' ? 'PEERLESS FINANCIAL SERVICES LIMITED' : $cheque5desc->field_value) : $cheque5value1 = 'PEERLESS FINANCIAL SERVICES LIMITED';
                ...
                // Witness2 full name
                case 'witness2fullname' :
                    $disabledField = '';
                    $extraClass = 'text-uppercase';
                    break;
                // Witness2 street address
                case 'witness2streetaddress' :
                    $disabledField = '';
                    $extraClass = 'text-uppercase';
                    break;
                // Witness2 street address
    ...
    switch ($type) {
        ...
        case 'file':
            ...
            break;
        case 'select':
            // when Highest education of the borrower appears
            $highestQualifications = DB::table('qualifications')->orderBy('name')->get();

            if ($key_name == "highesteducationoftheborrower") {
                $option = '<option value="" class="text-uppercase" selected hidden>Select Highest qualification of the Borrower</option>';
                foreach ($highestQualifications as $index => $val) {
                    $selected = '';
                    // if (strtolower($respValue) == strtolower($val)) $selected = 'selected';

                    $option .= '<option value="' . $val->name . '" ' . $selected . '>' . $val->name . '</option>';
                }
            ...
            } elseif ($key_name == "highesteducationofthecoborrower") {
                $option = '<option value="" class="text-uppercase" selected hidden>Select Highest qualification of the Co-borrower</option>';
                foreach ($highestQualifications as $index => $val) {
                    $selected = '';
                    // if (strtolower($respValue) == strtolower($val)) $selected = 'selected';
                ...
                $response = '<div class="w-100"><select class="form-control form-control-sm" name="field_name[' . $key_name . ']" ' . $required . ' ' . $disabledField . '>' . $option . '</select><input type="hidden" value="' . $field_id . '" name="field_id[' . $field_id . ']">' . $optionalFieldsData . '</div>';
            } elseif ($key_name == "highesteducationofthecoborrower2") {
                $option = '<option value="" class="text-uppercase" selected hidden>Select Highest qualification of the Co-borrower 2</option>';
                foreach ($highestQualifications as $index => $val) {
                    $selected = '';
                    // if (strtolower($respValue) == strtolower($val)) $selected = 'selected';
                ...
                $response = '<div class="w-100"><select class="form-control form-control-sm" name="field_name[' . $key_name . ']" ' . $required . ' ' . $disabledField . '>' . $option . '</select><input type="hidden" value="' . $field_id . '" name="field_id[' . $field_id . ']">' . $optionalFieldsData . '</div>';
            } elseif ($key_name == "highesteducationoftheguarantor") {
                $option = '<option value="" class="text-uppercase" selected hidden>Select Highest qualification of the Guarantor</option>';
                foreach ($highestQualifications as $index => $val) {
                    $selected = '';
                    // if (strtolower($respValue) == strtolower($val)) $selected = 'selected';
                ...
                $response = '<div class="w-100"><select class="form-control form-control-sm text-uppercase" name="field_name[' . $key_name . ']" ' . $required . ' ' . $disabledField . '>' . $option . '</select><input type="hidden" value="' . $field_id . '" name="field_id[' . $field_id . ']">' . $optionalFieldsData . '</div>';
            } else {
                ...
                $response = '<div class="w-100 text-uppercase"><select class="form-control form-control-sm text-uppercase" name="field_name[' . $key_name . ']" ' . $required . ' ' . $disabledField . '>' . $option . '</select><input type="hidden" value="' . $field_id . '" name="field_id[' . $field_id . ']">' . $optionalFieldsData . '</div>';
            }
        ...
        case 'checkbox':
            ...
            foreach ($expValue as $index => $val) {
                ...
                $option .= '<div class="single-checkbox-holder"><input class="form-check-input text-uppercase" type="checkbox" name="field_name[' . $key_name . '][]" id="' . $key_name . '-' . $index . '" value="' . $val . '" '.$checked.' '.$disabledField.'> <label for="' . $key_name . '-' . $index . '" class="form-check-label mr-1">' . $val.' </label></div>';
            }
            $response = '<div class="form-check text-uppercase">' . $option . '</div><input type="hidden" value="' . $field_id . '" name="field_id[' . $field_id . ']">';
            break;
        case 'radio':
            $expValue = explode(', ', $value);
            $option = '';
            foreach ($expValue as $index => $val) {
                ...
                $option .= '<input class="form-check-input text-uppercase" type="radio" name="field_name[' . $key_name . ']" id="' . $key_name . '-' . $index . '" value="' . $val . '" ' . $required . ' ' . $checked . ' '.$disabledField.'> <label style="white-space: break-spaces;" for="' . $key_name . '-' . $index . '" class="form-check-label mr-1">' . $val . '</label><input type="hidden" value="' . $field_id . '" name="field_id[' . $field_id . ']"> <div></div>  ';
            }
            $response = '<div class="w-100"><div class="form-check nowrap text-uppercase">' . $option . '</div>'.$optionalFieldsData.'</div>';
            break;
        case 'textarea':
            $response = '<textarea placeholder="' . $name . '" class="form-control form-control-sm text-uppercase" style="min-height:100px;max-height:200px" name="field_name[' . $key_name . ']" ' . $required . ' '.$disabledField.'>' . $respValue . '</textarea><input type="hidden" value="' . $field_id . '" name="field_id[' . $field_id . ']">';
            break;
        default:
...
{
    if($data['SubjectType'] =="Notify Mail"){
        $getDocuchecker = Docuchecker::where('id', $data['docuId'])->where('status', 1)->first();
        $rm_id = $getDocuchecker->rm_id;
        $reporting_manager = $getDocuchecker->RMData->reporting_manager;
        $borrower_id = $getDocuchecker->borrower_id;
        $from_address = Auth::user()->email;
        // $from_address = "rajib.ali@onenesstechs.in";
        // mail log
        $newMail = new DocuMail;
        $newMail->docuchecker_id = $data['docuId'];
        $newMail->RM_id = $rm_id;
        $newMail->customer_id = $borrower_id;
        $newMail->reporting_manager_id = $reporting_manager;
        $newMail->from = $from_address;
        $newMail->to = $data['RMEmail'];
        $newMail->ToCC = $data['ReportingManagerMail'];
        $newMail->subject = $data['SubjectType'];
        $newMail->save();
        // send mail
        Mail::send('mail/' . $data['blade_file'], $data, function ($message) use ($data, $from_address) {
            $message->to($data['RMEmail'], $data['RMName'])
                ->bcc($data['ReportingManagerMail'], $data['ReportingManagerName'])
                ->subject($data['subject'])
                ->from($from_address, env('APP_NAME'));
        });
    }else{
        $from_address = "digi.signature@peerlessfinance.in";
        // mail log
...
function avaialbleStampsAgreementWise($agreementId, $amount, $page_no) {
    $data = Estamp::where('used_in_agreement', $agreementId)->where('amount', $amount)->where('pdf_page_no', $page_no)->latest('id')->first();
    // echo "<pre>";
    // print_r($data);
    // die();
    return $data;
...
function specificStampWiseBorrowerDetails($stamp_id){
    ...
    if ($estamp_details->used_flag == 1) {
        ...
        $borrower_name = $borrower_details ? $borrower_details->full_name : 'Not Found';
...
//Function to fetch co-borrower and gurantor details
function coborrowerAndgurantor($id)
{
    $agreement_data = AgreementData::where('rfq_id',$id)->get();
    // dd($agreement_data);
    $cbg_det = [];

    $cbg_det['prefix_cb1'] = $cbg_det['name_cb1'] = $cbg_det['mobile_cb1'] = $cbg_det['email_cb1'] = '';
    $cbg_det['prefix_cb2'] = $cbg_det['name_cb2'] = $cbg_det['mobile_cb2'] = $cbg_det['email_cb2'] = '';
    $cbg_det['prefix_gur'] = $cbg_det['name_gur'] = $cbg_det['mobile_gur'] = $cbg_det['email_gur'] = '';

    foreach ($agreement_data as $value) {
        
        //Co-Borrower
        if($value->field_name == 'prefixofthecoborrower'){
            $cbg_det['prefix_cb1'] = $value->field_value;
        }
        if($value->field_name == 'nameofthecoborrower'){
            $cbg_det['name_cb1'] = $value->field_value;
        }
        if($value->field_name == 'mobilenumberofthecoborrower'){
            $cbg_det['mobile_cb1'] = $value->field_value;
        }
        if($value->field_name == 'emailidofthecoborrower'){
            $cbg_det['email_cb1'] = $value->field_value;
        }

        //Co-Borrower2
        if($value->field_name == 'prefixofthecoborrower2'){
            $cbg_det['prefix_cb2'] = $value->field_value;
        }
        if($value->field_name == 'nameofthecoborrower2'){
            $cbg_det['name_cb2'] = $value->field_value;
        }
        if($value->field_name == 'mobilenumberofthecoborrower2'){
            $cbg_det['mobile_cb2'] = $value->field_value;
        }
        if($value->field_name == 'emailidofthecoborrower2'){
            $cbg_det['email_cb2'] = $value->field_value;
        }


        //Guarantor
        if($value->field_name == 'prefixoftheguarantor'){
            $cbg_det['prefix_gur'] = $value->field_value;
        }
        
        if($value->field_name == 'nameoftheguarantor'){
            $cbg_det['name_gur'] = $value->field_value;
        }
        
        if($value->field_name == 'mobilenumberoftheguarantor'){
            $cbg_det['mobile_gur'] = $value->field_value;
        }
        
        if($value->field_name == 'emailidoftheguarantor'){
            $cbg_det['email_gur'] = $value->field_value;
        }
    }
    return $cbg_det;
}

function autoFillPanDet($x)
{
    $count = count(AgreementData::where('field_name','like','%pancardnumber%')->where('field_value','like','%'.$x.'%')->get());
    
    if($count <= 0)
        return "No-data";

    $data =  AgreementData::where('field_name','like','%pancardnumber%')->where('field_value','like','%'.$x.'%')->get()[0];
    
    $field_name = substr($data->field_name, 18);
    $rfq_id = $data->rfq_id;

    $detdata = AgreementData::where('rfq_id', $rfq_id)->where('field_name', 'like', '%the'.$field_name.'%')->get();
    $details_data = [];

    $details_data['streetadress'] = '';
    $details_data['pincode'] = '';
    $details_data['city'] = '';
    $details_data['state'] = '';

    foreach ($detdata as $value) {

        // Prefix
        if ($value->field_name == 'prefixofthe'.$field_name) {
            $details_data['prefix'] = $value->field_value;
        }

        // Name
        if ($value->field_name == 'nameofthe'.$field_name) {
            $details_data['name'] = $value->field_value;
        }

        // Street Address
        if ($value->field_name == 'streetaddressofthe'.$field_name) {
            $details_data['streetadress'] = $value->field_value;
        }

        // Pincode
        if ($value->field_name == 'pincodeofthe'.$field_name) {
            $details_data['pincode'] = $value->field_value;
        }

        // City
        if ($value->field_name == 'cityofthe'.$field_name) {
            $details_data['city'] = $value->field_value;
        }   

        // State
        if ($value->field_name == 'stateofthe'.$field_name) {
            $details_data['state'] = $value->field_value;
        }

        // Valid Document
        if ($value->field_name == 'officiallyvaliddocumentsofthe'.$field_name) {
            $details_data['validdoc'] = $value->field_value;
        }

        // Occupation
        if ($value->field_name == 'occupationofthe'.$field_name) {
            $details_data['occupation'] = $value->field_value;
        }

        // Resident status
        if ($value->field_name == 'residentstatusofthe'.$field_name) {
            $details_data['resident_status'] = $value->field_value;
        }

        // dob 
        if ($value->field_name == 'dateofbirthofthe'.$field_name) {
            $details_data['dob'] = $value->field_value;
        }

        //Material status
        if ($value->field_name == 'maritalstatusofthe'.$field_name) {
            $details_data['material_status'] = $value->field_value;
        }

        // Highest Eduction
        if ($value->field_name == 'highesteducationofthe'.$field_name) {
            $details_data['highest_ed'] = $value->field_value;
        }

        //Mobile number
        if ($value->field_name == 'mobilenumberofthe'.$field_name) {
            $details_data['mobile'] = $value->field_value;
        }

        //Email
        if ($value->field_name == 'emailidofthe'.$field_name) {
            $details_data['email'] = $value->field_value;
        }
    }
    
    // dd(json_encode($details_data));
    
    return json_encode($details_data);
}

function CheckVernaculationDataSet($id)
{
    if(count(DB::table('vernacular_declaration_data')->where('borrower_agreement_id',$id)->get()) > 0){
        return true;
    } 
    return false;
}

function CheckVernaculationData($id)
{
    if(count(DB::table('vernacular_declaration_data')->where('borrower_agreement_id',$id)->get()) > 0){
        $d = DB::table('vernacular_declaration_data')->where('borrower_agreement_id',$id)->get()[0];
        if($d->is_required == 1 && ($d->borrower_check == 'Yes' || $d->coborrower_check == 'Yes' || $d->co2borrower_check == 'Yes' || $d->gurrantor_check == 'Yes'))
            return true;
        else
            return false;
    } 
...
function VernaculationData($id){
    $data = DB::table('vernacular_declaration_data')->where('borrower_agreement_id',$id)->get();
    if(count($data) > 0) {
        return (array)$data[0];
    }
    else return [
        ...
        'executant_adress' => '',
        'executant_email' => '',
        'executant_phone_number' => '',
        'is_executant_fathers_or_husband' => '',
        'executant_fathers_or_husband_name' => '',

        'vernacular_language' => '',

        'borrower_check' => '',
        'borrower_father_or_husband_name' => '',
        'is_borrower_father_or_husband' => '',

        'coborrower_check' => '',
        'coborrower_father_or_husband_name' => '',
        'is_coborrower_father_or_husband' => '',
        
        'co2borrower_check' => '',
        'co2borrower_father_or_husband_name' => '',
        'is_co2borrower_father_or_husband' => '',
        
        'gurrantor_check' => '',
        'gurrantor_father_or_husband_name' => '',
        'is_gurrantor_father_or_husband' => '',

        'executant_know_borrower_years' => '',
        'executant_know_coborrower_years' => '',
        'executant_know_coborrower2_years' => '',
        'executant_know_gurrantor_years' => '',
    ];
}

function getApplicationID($bid, $agreements)
{
    $app_arr = [];
    if(count($agreements) > 0){
        foreach ($agreements as $value) {
            if(BorrowerAgreement::find($value->id)->application_id != null)
                return "<i class='text-sm text-bold'>".BorrowerAgreement::find($value->id)->application_id."</i><br>";
            else
            return false;
        }
    }
...
function getEsigns($rfq_id)
{
    return ESignature::where('rfqId',$rfq_id)->where('unique_id','like','%HMUID%')->groupBy('unique_id')->get();
}

function getEsignDetails($uniq_id)
{
    return ESignature::where('unique_id',$uniq_id)->get();
}

function getWebHookResponses($uniq_id)
{
    return DB::table('zoop_webhook_response')->where('esign_uniq_id',$uniq_id)->get();
}

function fetchAgreement($request_id)
{
    $curl = curl_init();

    curl_setopt_array($curl, array(
    CURLOPT_URL => 'https://live.zoop.one/contract/esign/v5/fetch/request?request_id='.$request_id,
    ...
    CURLOPT_HTTPHEADER => array(
        'app-id: 63904d748b5c20001ee3ff12',
        'api-key: YVB63N3-PD746X9-QEH9W7W-7WK84A8'
    ),
    ));
