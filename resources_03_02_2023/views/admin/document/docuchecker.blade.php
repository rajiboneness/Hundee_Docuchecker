                ...
                <div class="card">
                    <div class="card-header">
                        <h5 class="font-weight-light text-dark">
                            <span class="font-weight-normal" title="Borrower's name">
                                {{ $Borrower->name_prefix }} {{ $Borrower->full_name }}
                            </span> - <span title="Agreement name">{{ $Borrower->Loan_Type }} </span><span class="badge badge-{{ $dataCount<1 ? "success" : ""  }}">{{ $dataCount<1 ? "Verified" : "" }}</span>
                        </h5>
                    ...
                    <div class="card-body">
                        <table class="table table-sm table-bordered table-hover" id="borrowers-table">
                            ...
                            <tbody>
                                @foreach($finalLoanType as $key => $parentVal)
                                @php
                                    $getDoc = App\Models\Document::where('id', $parentVal)->first();
                                @endphp
                                <table style="border:1px solid #ddd; width:100%;background: #25517f;" class="text-light">
                                    <tr colspan="5">
                                        <td class="px-2"><strong>{{ $getDoc->document_name }}</strong></td>
                                    </tr>
                                </table>
                                <table class="table table-sm table-bordered table-hover">
                                    @foreach($loanDocument as $docKey => $docuVal)
                                        @php
                                        $MatchingData = $docuVal->documentData->parent_id;
                                        @endphp
                                        @if($MatchingData ==$parentVal)
                                            <tr>
                                                <td width="4%">{{ $key+1 }}</td>
                                                <td width="40%">{{ $docuVal->document_name}}</td>
                                                <td>
