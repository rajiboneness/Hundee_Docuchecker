                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-md-6 text-sm-left">
                                    <div class="mb-2">
                                        @php
                                        // $CustomerLD = App\Models\CustomerLoanDocument::where('borrower_id', $data->borrower_id)->where('status', 1)->get();

                                        $CustomerLD = App\Models\CustomerLoanDocument::where('borrower_id', $data->borrower_id)->get();
                                        $route = '';

                                        $CustomerLoanDocument = count($CustomerLD);

                                        if ($CustomerLoanDocument == 0) {
                                            $color = 'danger';
                                            $icon = '<i class="fas fa-info-circle mr-2"></i>';
                                            $route = route('user.docu-checker.customer', ['customer' => $data->full_name]);
                                        } else {
                                            
                                            $Pending = App\Models\CustomerLoanDocument::where('borrower_id', $data->borrower_id)->where('status', 1)->get();
                                            ...
                                            $route = route('user.docu-checker.index', [$data->borrower_id]);
                                        }
                                        @endphp

                                        <a href="{{ $route }}" class="mb-1 btn {{auth()->user()->user_type == 1 || auth()->user()->user_type == 3 ? '' : 'disabled'}} btn-sm btn-{{$color}}">
                                            {!! $icon !!}
                                           Docu Checker
                                        </a>
                                        @if(!empty($DocumentsPending))
                                        @php
                                            $href = $DocumentsPending > 0 ? '#DocuChekerModal' : route('user.borrower.setAnnexure5doc', [$id]);
                                            $modal = $DocumentsPending > 0 ? "modal" : "" ;
                                            $target = $DocumentsPending > 0 ? "#DocuChekerModal" : "#vernacularDeclarationModal";
                                            $stamphref = $DocumentsPending > 0 ? "#DocuChekerModal" : "#viewPdfModal";
                                            $pendingData = $DocumentsPending > 1 ? "s are" : " is";
                                            $pendingDataCount = $DocumentsPending;
                                        @endphp
                                        @else
                                        @php 
                                            $href = route('user.borrower.setAnnexure5doc', [$id]);
                                            $modal = "";
                                            $target = "#vernacularDeclarationModal";
                                            $stamphref = '#viewPdfModal';
                                            $pendingData = "";
                                            $pendingDataCount = "";
                                        @endphp
                                        @endif
                                        <a href="{{ $href }}" class="mb-1 btn {{auth()->user()->user_type == 1 || auth()->user()->user_type == 3 ? '' : 'disabled'}} btn-sm btn-{{ $data->annX5dataCount == 0 ? 'danger' : 'success' }}" data-toggle="{{ $modal }}" rel="1" id="{{route('user.borrower.setAnnexure5doc', [$id]) }}" onclick="GetDetails(this.id,this.rel)">
                                        ...
                                        <a href="javascript:void(0)" class="mb-1 btn {{auth()->user()->user_type == 1 || auth()->user()->user_type == 3 ? '' : 'disabled'}} btn-sm btn-{{CheckVernaculationDataSet($id) ? 'success' : 'danger'}}" data-toggle="modal"
                                            data-target="{{ $target}}" id="#vernacularDeclarationModal" rel="2" onclick="GetDetails(this.id,this.rel)">
                                        ...
                                        {{-- Docu Checker Modal --}}
                                        <div class="modal fade" id="DocuChekerModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                            <div class="modal-dialog" role="document">
                                              <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <span><strong>{{ $pendingDataCount }} Document{{ $pendingData }} not submitted yet</strong></span><br>
                                                    <span> Would you want to proceed with this status ?</span>
                                                </div>
                                                <div class="modal-footer">
                                                  {{-- <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button> --}}
                                                  
                                                  <a href="{{ route('user.docu-checker.index', [$data->borrower_id]) }}" class="btn btn-primary">Submit Now</a>
                                                  <a id="SkipBtn" alt="DocuChekerModal" href="" data-toggle="" class="btn btn-success" onclick="Skipdetails(this.alt)">Proceed</a>
                                                </div>
                                              </div>
                                            </div>
                                          </div>
                                        ...
                                        @if ($data->stampPaper1 && $data->stampPaper2 && $data->stampPaper3 && $data->stampPaper4)
                                            ...
                                            <a href="{{ $stamphref }}" data-toggle="modal" class="btn btn-sm btn-success {{auth()->user()->user_type == 1 || auth()->user()->user_type == 3 ? '' : 'disabled'}}" rel="3" id="#viewPdfModal" onclick="GetDetails(this.id,this.rel)">
                                                <i class="fas fa-check-circle mr-2"></i> Stamp Paper
                                            </a>
                                        @else
                                            <a href="{{ $stamphref }}" data-toggle="modal" class="btn btn-sm btn-danger {{auth()->user()->user_type == 1 || auth()->user()->user_type == 3 ? '' : 'disabled'}}" rel="4" id="#viewPdfModal" onclick="GetDetails(this.id,this.rel)">
...
@section('script')
    ...
    <script>
        function GetDetails(id, rel){
            $('#SkipBtn').attr('href', id);
            if(rel > 1){
                $('#SkipBtn').attr('data-toggle', 'modal');
            }
        }
        function Skipdetails(alt){
            $('#DocuChekerModal').modal('toggle');
        }
