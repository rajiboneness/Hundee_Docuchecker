...
class PDFController extends Controller
{
    ...
    public function getData($agreement, $checkFeld,$image = false){
        ...
        foreach ($agreement as $key => $value) {
            if($value->field_name == $checkFeld){
                ...
                if($checkFeld != 'documentstobeattachedwithapplicationforloan' || $checkFeld != 'otherdocumentstobeattachedwithapplicationforloan')
                    $response = strtoupper($value->field_value);
                break;
            }
        }
    ...
    public function showDynamicPdf(Request $request, $borrowerId, $agreementId, $borrowerAgreementsId)
    {
        // dd($request->all());
        $data = (object)[];
        $agreement = AgreementData::join('agreement_rfqs', 'agreement_data.rfq_id', '=', 'agreement_rfqs.id')->where('borrower_id', $borrowerId)->where('agreement_id', $agreementId)->latest('agreement_rfqs.id')->get();
        
        $annexureImages = AnnexureVData::where('borrower_agreement_id',$borrowerAgreementsId)->latest('id')->get();
        // dd($agreement); 
        ...
        $customBorrowerName = str_replace(' ', '-', strtoupper($this->getData($agreement, 'nameoftheborrower')));
        ...
        //page 28
        $data->annexurefiveimagepageone = count($annexureImages) > 0 ? $annexureImages[0]->page_one_data : 'No data uploaded!';

        //page 29
        $data->annexurefiveimagepagetwo = count($annexureImages) > 0 ? $annexureImages[0]->page_two_data : 'No data uploaded!';
        ...
        $data->borrower_agreement_data = BorrowerAgreement::find($borrowerAgreementsId);
