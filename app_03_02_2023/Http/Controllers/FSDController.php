...
class FSDController extends Controller
{
    ...
    {
        $data = (object)DB::select('select * from fsd_docments where status = ?', [1]);
        // dd($data);
        return view('admin.fsd.index', compact('data'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if (!$validator->fails()) {
            $data = DB::select('select * from fsd_docments where name = ?', [$request->name]);
            if (count($data) == 0) {
    ...
    {
        // dd("Comming Here");
        $data = (object)DB::select('select * from fsd_docments where id = ?', [1])[0];
    ...
    public function update(Request $request)
    {
        ...
        if (!$validator->fails()) {
            DB::update('update fsd_docments set name = ? where id = ?', [$request->name, $request->id]);
    ...
    {
        DB::delete('delete from fsd_docments where id = ?', [$request->id]);
