...
class ZoopController extends Controller
{
    ...
    public function esignSubmit(Request $request) {
        ...
        $request->validate([
            ...
            'guarantor_city' => 'nullable',
            'guarantor_purpose' => 'nullable',
        ...
        $guarantor_purpose = $request->guarantor_purpose ?? '';
        ...
        /** Guarantor */
        if(!empty($request->guarantor)){
            if (!empty($guarantor_name)) {
                $esign7 = new ESignature();
                $esign7->rfqId = $request->rfqId;
                $esign7->unique_id = $request->unique_id;
                $esign7->encoded_pdf = $converted_doc;
                $esign7->document_info = $document_info;
                $esign7->user_type = "guarantor";
                $esign7->user_name = $guarantor_name;
                $esign7->user_email = $guarantor_email;
                $esign7->user_city = $guarantor_city;
                $esign7->user_purpose = $guarantor_purpose;
                $esign7->save();
                ...
                $data = [
                    ...
                    'city' => $guarantor_city,
                    'blade_file' => 'esign/index',
                    'route' => route('zoop.guarantor.mail.click', $esign7->id)
                ];
                $resp = SendMail($data);
            }
        }  
    ...
    public function guarantorSign(Request $request, $id) {
        // dd($request->all());
        $esigning = ESignature::findOrFail($id);
        $doc = $esigning->encoded_pdf;
        $info = $esigning->document_info;
        ...
        if(VernaculationData($BorrowerAgreement)['gurrantor_check'] == 'Yes'){
            ...
            array_push($guar_pos_arr, json_encode($barr));
        }

        $guar_pos_string = implode(',',$guar_pos_arr);
